...
- **The step 1 carry-overs, closed.**
  - **`ropa-db` takes no external connections** (`ipAllowList: []`). Render's
    spec: an empty list blocks all external connections; leaving the key out
    keeps what the database already allows. Blueprint changes sync
    automatically on push unless **Auto Sync** is off on the Blueprint's
    Settings page, which nothing in the repository records.
  - **Each tool reads only its own configuration:** the service everything,
    the migrator and seed `NODE_ENV`/`LOG_LEVEL`/`DATABASE_URL`, `demo:data`
    the mint secret, a base URL and a subject. One exit-on-error helper in
    `startup.ts`.
  - **`demo:data` loads the whole story over HTTP.** It matches activities by
    name, because over HTTP the server allocates codes and a create cannot
    be rolled back, so on a busy service the story's P1 may be P5. It matches
    edits by change note in `/revisions`, and runs twice in a test against a
    real listening server; making edits skip that check failed the test.
    `inputFromActivity` (package) is the API-output twin of
    `inputFromSnapshot` (domain).
- **Where today's decisions live for later steps:** `ropa-api.md` §8 ("Decided
  during build step 2"), the as-built notes in §5.1 and §5.2, §9 question 6,
  and DM §3.8 ("active"), so they survive this plan being archived.
