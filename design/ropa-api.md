...
| Auth | Bearer JWT (§1.9). Reads are public by default; writes need a token |
...
|---|---|---|
| Comes from | The token's `sub` claim (§1.9) | `changeNote` field in the request body, e.g. `"Added Scribe AI for CV parsing"` |
| Required? | Yes: every write needs a token | No (optional for now; see DM §11, F4) |
| Can the caller choose it? | **No.** `X-Actor` is ignored whenever a token is present, and only honoured when auth is switched off for local development (`AUTH_DISABLED=true`) | Yes, it's the caller's explanation |
| Stored in | `revision.actor` | `revision.change_note` |

An actor that callers could set freely would undermine the point of the history: the Chapter 8 regulator scene relies on "who changed this" being authenticated, not asserted.
...
| 401 | Missing, expired or invalid token (§1.9) |
| 403 | Valid token without the required permission; the response names it (§1.9) |
| 404 | Unknown `{ref}` |
...
Demo-scale authentication: short-lived JSON Web Tokens and role-based permissions. No sessions, no SSO, no user management.

**Principals.** The known subjects live in one environment variable, `PRINCIPALS`, as JSON. No user table (the upgrade path is DM §11, F5):
...
**Minting.** `POST /v1/tokens` with `{ "subject": "priya.raman", "secret": "…" }`, checked against `TOKEN_MINT_SECRET`, returns the token plus the subject's roles and effective permissions. Unknown subject or wrong secret: `401`. The endpoint is rate-limited.

**The token.** HS256, signed with `JWT_SECRET` (generated by the Blueprint with `generateValue: true`, so it never appears in the repository). Claims: `sub`, `name`, `roles`, `iss: ropa`, `aud: ropa-api`, `iat`, `jti`, and `exp` **8 hours** out. Sent as `Authorization: Bearer <token>`. A short expiry is the "automatic logoff" control from Render's HIPAA guidance.

**What needs a token**

| Requests | Token |
|---|---|
| `GET /healthz`, `GET /openapi.json`, `GET /api-docs`, `POST /v1/tokens` | Never |
| Any other `GET` | Optional by default: the public demo is readable, and an anonymous caller is treated as `viewer`. Set `REQUIRE_AUTH_FOR_READS=true` to close that |
| Everything that writes | **Required** |

**Permissions.** Each route declares the one permission it needs. Roles are named bundles of permissions, and a token's roles are combined.

| Group | Permissions |
|---|---|
| Records | `record:read`, `record:write`, `record:delete`, `taxonomy:write`, `system:write` |
| Lifecycle | `activity:approve` (activate and retire) |
| Review items | `review:read`, `review:create`, `review:resolve` |
| Views | `view:report`, `view:subprocessors`, `view:impact`, `view:datamap`, `view:coverage` |
| History | `history:read` (revisions and `/changes`) |

| Role | Permissions |
|---|---|
| `viewer` | `record:read`, `history:read`, `review:read`, all `view:*` |
...
- **Services get their own narrow roles.** Render describes its private network as a workspace-level trust model, not zero-trust, so services authenticate at the application layer like anyone else. The Monitor's token can read impact and open review items, and nothing else.

**Failures**
- `401` — missing, expired or invalid token, or a write attempted anonymously.
- `403` — valid token without the permission. The problem details name it: `"requiredPermission": "activity:approve"`, so the control is visible rather than mysterious.

**Who made the change.** The actor comes from the token's `sub`, not from a header (§1.6).

**`GET /v1/me`** returns the subject, name, roles and effective permissions, so a UI can hide actions it isn't allowed to take.

**In OpenAPI:** a `bearerAuth` security scheme (so Swagger UI gets an Authorize button), and each operation documents its required permission, also exposed as an `x-required-permission` extension.

**Not included** (future): per-client scoping (a token that only sees Aurelia's data), refresh tokens, sessions, SSO and SCIM, a principals table, and key rotation. Rotating `JWT_SECRET` simply invalidates every existing token, which is acceptable for a demo.
...
| Endpoint | Purpose |
|---|---|
| `POST /tokens` | Mint a token for a known subject |
| `GET /me` | The caller's subject, roles and effective permissions |
...
2. ~~**Actor and change note**~~ **Resolved (2026-09-19):** the change note is a permanent, optional, write-only `changeNote` body field (§1.6). **Updated the same day:** the actor now comes from the JWT's `sub` (§1.9), so `X-Actor` only exists for local development with auth switched off.
